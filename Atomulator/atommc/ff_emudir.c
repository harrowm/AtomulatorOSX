...
int validSFN(char *filename)
{
	int	result = 1;	
	int dotpos = -1;
	int beforedot = 0;  // MH - initialisation required for OSX
	int afterdot = 0;   // MH - initialisation required for OSX
	int CharNo;
...
int findclose(EMUDIR	*dir)
{
    ...
    // MH - Had to add in the check for NULL to avoid runtime crash on OSX
    // Had to return a value to enable compile on OSX
...
BYTE get_fat_attribs(int	fid)
{
    ...
    return result; // MH - added to stop xcode complaints; looks like a bug
}
