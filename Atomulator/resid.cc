/*Atomulator v1.10 by Tom Walker
  resid-fp interfacing code*/

#include <stdio.h>
#include <string.h>
#include <stdint.h>
#include "SID.h"   // chnaged MH
#include "siddefs-fp.h"
#include "sidtypes.h"
#include "sid_atom.h"

int sidrunning=0;
extern "C" void sid_initd();
extern "C" void sid_resetd();
...
extern "C" void sid_fillbuf(int16_t *buf);
...
void sid_init()
{
        ...
        // MH HACK psid->sid->set_voice_nonlinearity(1.0f);
        // MH HACK psid->sid->get_filter().set_distortion_properties(0.f, 0.f, 0.f);
        // MH HACK psid->sid->get_filter().set_type4_properties(6.55f, 20.0f);

        psid->sid->enableFilter(true);
        // MH HACK psid->sid->enable_external_filter(true);
    ...
    psid->sid->setSamplingParameters((float)cycles_per_sec, reSIDfp::DECIMATE, (float)31250, 0.9*31250.0/2.0);
}
...
void sid_fillbuf(int16_t *buf)
{
    ...
    if (sidrunning)
    {
        c=psid->sid->clock(64, buf);
        //printf("Clocked %d buf %d\n", c, *buf);
    }
}
