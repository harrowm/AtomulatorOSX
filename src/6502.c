...
void dumpram()
{
	...
	extern ALLEGRO_PATH *exepath;
	al_set_path_filename(exepath, "ram.dmp");
	strlcpy(fname, al_path_cstr(exepath, ALLEGRO_NATIVE_PATH_SEP), MAXPATH - 1);
...
static void load_rom(char	*Name,
			  int 	Size,
	          int 	Offset)
{
	FILE    *RomFile;
    ...
    // Get the directory for the ROM files in an OSX/Linux/Windows friendly manner
    al_append_path_component(exepath, "roms");
    al_set_path_filename(exepath, Name);
    strlcpy(filename, al_path_cstr(exepath, ALLEGRO_NATIVE_PATH_SEP), MAXPATH-1);
	al_drop_path_tail(exepath); // leave clean for next use
	...
	if(RomFile!=NULL)
	{
		...
		rpclog("ERROR: Loading rom from %s failed !\n", filename);
}

void loadroms()
{
	load_rom("akernel.rom",		ROM_SIZE_ATOM, ROM_OFS_AKERNEL);
	load_rom("dosrom.rom",		ROM_SIZE_ATOM, ROM_OFS_DOSROM);
	load_rom("afloat.rom",		ROM_SIZE_ATOM, ROM_OFS_AFLOAT);
	load_rom("abasic.rom",		ROM_SIZE_ATOM, ROM_OFS_ABASIC);
	load_rom("axr1.rom",		ROM_SIZE_ATOM, ROM_OFS_UTILITY);
	load_rom("ramrom.rom",		RAM_ROM_SIZE,  ROM_OFS_RAMROM);
	load_rom("gdos2015.rom",	ROM_SIZE_GDOS2015, ROM_OFS_GDOS2015);
}
...
extern uint8_t fetcheddat[32];

// The compiler has trouble optimizing this form of case statement.
// Handcoded using ifs below
//
...
uint8_t readmeml(uint16_t addr)
{
    ...
    if (i == 0xB400) return ReadMMC(addr);  /* MMC      */
    ...
    if (i <= 0xCC00) return abasic_ptr[addr & 0x0FFF];  /* BASIC              */
    if (i <= 0xDC00) return afloat_ptr[addr & 0x0FFF];  /* Floating point ROM */
    if (i <= 0xEC00) return dosrom_ptr[addr & 0x0FFF];  /* Disc ROM           */
...
void exec6502(int linenum, int cpl)
{
    ...
    //                rpclog("Exec line %i\n",lines);
        lns = lines;
        /*
            Hack to fix an issue where first displayed line has garbage,
            so the user can choose if they want the more accurate display,
