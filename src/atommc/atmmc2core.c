...
// AVR / Atomulator don't currently have spare port so don't compile it !
            ...
            else if (received == CMD_READ_PORT) // read portb
            {
                ...
                if (joystick != NULL)
                {
                    ...
                    // MH - my joystick seems to bounce around 0 .. so 0.1 stabilizes the joystick output
                    if (joystate.stick[0].axis[0] > 0.1) // right
                        JOYSTICK ^= 1;
                    if (joystate.stick[0].axis[0] < -0.1) // left
                        JOYSTICK ^= 2;
                    if (joystate.stick[0].axis[1] > 0.1) // down
                        JOYSTICK ^= 4;
