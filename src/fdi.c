...
{
	if (fdih[drive])
		fdi2raw_header_free(fdih[drive]);
	if (fdif[drive])
		fclose(fdif[drive]);
	fdif[drive] = NULL;
}
...
//        printf("Track start %i\n",track);
	if (track < 0)
		track = 0;
	if (track > fdilasttrack[drive])
		track = fdilasttrack[drive] - 1;
	...
	if (!c)
		memset(ftrackinfo[drive][0][0], 0, ftracklen[drive][0][0]);
	...
	if (!c)
		memset(ftrackinfo[drive][0][1], 0, ftracklen[drive][0][1]);
	if (fdisides[drive])
	{
		...
		if (!c)
			memset(ftrackinfo[drive][1][0], 0, ftracklen[drive][1][0]);
		...
		if (!c)
			memset(ftrackinfo[drive][1][1], 0, ftracklen[drive][1][1]);
	}
	else
	{
...
//                        printf("Not found!\n");
	...
	if (pollbitsleft)
	{
		...
		if (!pollbitsleft)
		{
			pollbytesleft--;
			if (pollbytesleft)
				pollbitsleft = 16;          /*Set up another word if we need it*/
			if (readidpoll)
			{
				fdisectordat[5 - pollbytesleft] = decodefm(fdibuffer);
				if (fdireadaddr && pollbytesleft > 1)
					fdcdata(fdisectordat[5 - pollbytesleft]);
				if (!pollbytesleft)
				{
